...
if ( $product->is_in_stock() ) : ?>
	...
	<form class="cart flex flex-col gap-4" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
		...
		<div class="main-product-btns flex flex-col sm:grid items-start sm:items-center gap-3 py-4">
			<button type="submit" name="add-to-cart" value="<?php echo esc_attr( $product->get_id() ); ?>" class="single_add_to_cart_button owl-add-to-cart-btn alt<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?> w-full sm:w-auto theme-btn btn-secondary">

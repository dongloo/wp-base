...
<?php if (have_posts()) : ?>
    <?php while (have_posts()) : the_post(); setpostview(get_the_id()); ?>
        <?php
            $thuoc_du_an = get_post_meta(get_the_ID(), 'thuoc_du_an', true);
            ...
            $phan_loai_terms = get_the_terms(get_the_ID(), 'tin-tuc');
        ...
        <section class="py-8">
            <div class="container">
                <div class="flex flex-col lg:flex-row gap-8 items-start">
                                ...
                                <?php if (has_post_thumbnail( get_the_ID()) ): ?>
                                    <div class="news-detail-thumb">
                                        <div class="ratio-thumb ratio-16x9 rounded overflow-hidden">
                                            <?php owlGetPostThumb(get_the_ID(),'single-post-thumb','ratio-media', true, '', false); ?>
                                        </div>
                                    </div>
                                <?php endif; ?>                                   
                            </div>
                    ...
                    <div class="sidebar-area article-sidebar-area flex flex-col gap-3 lg:gap-6 order-first lg:order-none">
                        <?php if($thuoc_du_an): ?>
                        <aside>
                        ...
                        <aside>
                            ...
                            <a href="" class="filter-group-item border-0 cursor-pointer rounded-lg overflow-hidden w-full block">
                                <span class="px-4 py-2 lg:py-3 rounded-lg overflow-hidden bg-white text-sm text-center block text-dark-secondary">
                                <?php
                                if ($phan_loai_terms && !is_wp_error($phan_loai_terms)) {
                                    foreach ($phan_loai_terms as $term) {
                                        // Display or process each term
                                        echo $term->name;
                                    }
                                }
                                ?>                                    
